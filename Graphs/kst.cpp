#include "kst.h"
void PrintMCST(int T[][V - 1], vector<int> A[3])
{
...
void KruskalsMCST(Graph g)
{
    ...
    for (int i = 0; i < g.vertices.size(); i++)
    {
        ...
        while (p != NULL)
        {
            if (p->cost != 0 && !(find(A[2].begin(), A[2].end(), p->cost) != A[2].end()))
            {
                A[0].push_back(g.vertices[i]->data);
                A[1].push_back(p->data);
                A[2].push_back(p->cost);
            }
            p = p->next;
        }
    }
